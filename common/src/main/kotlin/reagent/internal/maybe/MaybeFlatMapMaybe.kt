...
internal class MaybeFlatMapMaybe<U, D>(val upstream: Maybe<U>, val func: (U) -> Maybe<D>) : Maybe<D>() {
  override fun subscribe(subscriber: Subscriber<D>) = upstream.subscribe(Operator(
      subscriber, func))

  class Operator<U, D>(val downstream: Subscriber<D>, val func: (U) -> Maybe<D>) : Subscriber<U> {
