...
class ManySourceTest {
  ...
  @Test fun fromCallable() {
    val called = AtomicBoolean()
    Many.fromCallable(Callable { called.getAndSet(true) })
        .testMany {
          item(false)
          complete()
        }
  ...
  @Test fun fromCallableThrowing() {
    val exception = RuntimeException("Oops!")
    Many.fromCallable(Callable { throw exception })
        .testMany {
          error(exception)
        }
  }
  ...
  @Test fun fromRunnable() {
    val called = AtomicBoolean()
    Many.fromRunnable<Any>(Runnable { called.set(true) })
        .testMany {
          complete()
        }
    Assert.assertTrue(called.get())
  ...
  @Test fun fromRunnableThrowing() {
    val exception = RuntimeException("Oops!")
    Many.fromRunnable<Any>(Runnable { throw exception })
        .testMany {
          error(exception)
        }
  }
